...
def registo_post():
    ...
    cliente = Cliente(nome=nome, apelido=apelido, email=email, password=password, admin=admin, profile_img=profile_img_path)
    session.add(cliente)
    session.commit()
...
def admin():
    if 'name' in flask_session:
        cliente = session.query(Cliente).filter(Cliente.email==flask_session['name']).first()
        return render_template('private/perfil.html', cliente=cliente)
    else:
...
def admin_post():
    ...
    # se houver uma nova profile_img
    if profile_img.filename != '':
        # guardar essa nova profile_img 
        profile_img_filename = f'{str(uuid1())}_{secure_filename(profile_img.filename)}'
        profile_img_path = f'private/assets/img/user_profile/{profile_img_filename}'
        profile_img.save(f'static/{profile_img_path}')
        
        # apagar a profile_img anterior, caso não seja a default (antes de atribuir o novo path)
        if cliente.profile_img != 'private/assets/img/user_profile/user_profile_default.jpg':
            os.remove(f'static/{cliente.profile_img}')

        cliente.profile_img = profile_img_path
...
def apagar_user(user_id):
    cliente_query = session.query(Cliente).filter(Cliente.id==user_id)
    cliente = cliente_query.first()
    respetivos_alugueres = session.query(Aluguer).filter(Aluguer.cliente_id==cliente.id)

    respetivos_alugueres.delete(synchronize_session=False)
    cliente_query.delete(synchronize_session=False)
    session.commit()
    return redirect(url_for('index'))
    
@app.route('/apagar-imagem')
def apagar_imagem():
    if 'name' in flask_session:
        cliente = session.query(Cliente).filter(Cliente.email==flask_session['name']).first()
        # se a profile_img não for a default, então apagá-la e alterá-la para a default
        if cliente.profile_img != 'private/assets/img/user_profile/user_profile_default.jpg':
            os.remove(f'static/{cliente.profile_img}')
            cliente.profile_img = 'private/assets/img/user_profile/user_profile_default.jpg'
            session.commit()
        return redirect(url_for('admin'))
    else:
        return render_template('public/404.html')
...
def admin_motas():
    if 'name' in flask_session:
        motas = session.query(Mota).all()
        cliente = session.query(Cliente).filter(Cliente.email==flask_session['name']).first()
        return render_template('private/motas.html', motas=motas, route='admin_motas', cliente=cliente)
    else:
        return render_template('public/404.html')
...
def adicionar_mota_post():
    ...
    if imagem1.filename == '':
        ...
        imagem1_filename = f'{str(uuid1())}_{secure_filename(imagem1.filename)}'
        imagem1_path = f'public/assets/img/motas/{imagem1_filename}'
        print(imagem1_path)
        imagem1.save(f'static/{imagem1_path}')
        mota = Mota(marca=marca, modelo=modelo, cilindrada=cilindrada, ano=ano, preco=custo, categoria_id=categoria_id, imagem1=imagem1_path)
...
def editar_mota(mota_id):
    ...
    categorias = session.query(Categoria).all()
    mota = session.query(Mota).filter(Mota.id==mota_id).first()
    return render_template('private/motas.html', motas=motas, mota_selecionada=mota, categorias=categorias, route='editar_mota')
...
def editar_mota_post(mota_id):
    ...
    mota.ano = ano
    mota.preco = preco
    mota.categoria_id = categoria_id
    session.commit()
...
# GESTAO DE ALUGUERES ------------------------------
@app.route('/admin/alugueres')
def admin_alugueres():
    if 'name' in flask_session:
        alugueres = session.query(Aluguer).all()
        cliente = session.query(Cliente).filter(Cliente.email==flask_session['name']).first()
        return render_template('private/alugueres.html', alugueres=alugueres, route='admin_alugueres', cliente=cliente)
    else:
        return render_template('public/404.html')

@app.route('/apagar-aluguer/<int:aluguer_id>')
def apagar_aluguer(aluguer_id):
    aluguer_query = session.query(Aluguer).filter(Aluguer.id==aluguer_id)
    aluguer = aluguer_query.first()
    mota = session.query(Mota).filter(Mota.id==aluguer.mota_id).first()
    
    mota.is_rented = 0
    aluguer_query.delete(synchronize_session=False)
    session.commit()
    return redirect(url_for('admin_alugueres')) 
