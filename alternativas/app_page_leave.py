...
def adicionar_mota():
    ...
    if retornar('sessao'):
        return render_template('private/adicionar_mota.html', categorias=categorias)
    else:
...
def editar_mota(mota_id):
    categorias = session.query(Categoria).all()
    mota = session.query(Mota).filter(Mota.id==mota_id).first()
    return render_template('private/editar_mota.html', mota_selecionada=mota, categorias=categorias)
